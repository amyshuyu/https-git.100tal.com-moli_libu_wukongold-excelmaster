...
class Manage extends Admin_Controller {
	...
	function login()
	{
		...
		if(isset($_POST['username'])) {
			...
			if(!$r) exit(json_encode(array('status'=>false,'tips'=>' 没有此用户名')));
			...
			if($r['password'] != $password) {
				if($rtime && $rtime['failure_times'] < $maxloginfailedtimes) {
					...
					$this->Times_model->update(array('login_ip'=>$ip,'is_admin'=>1,'failure_times'=> $rtime['failure_times']+1),array('username'=>$username));
				}elseif($rtime && $rtime['failure_times'] == $maxloginfailedtimes){
                    $this->Member_model->update(array('is_lock'=>1),array('username'=>$username));
                    exit(json_encode(array('status'=>false,'tips'=>'密码错误次数过多，您的帐号已被锁定！')));
                }else {
